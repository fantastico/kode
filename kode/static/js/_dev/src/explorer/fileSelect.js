define(function (require, exports) {
        ...
        var __list_position = function () {
            if (Global.fileListSelectNum == 1) {//只有一个为选中状态
                ...
                switch (pose) {
                    case "up":
                    case "left":
                        position = ((firstNumber <= 0) ? firstNumber : firstNumber - 1);
                        break;
                    case "down":
                    case "right":
    ...
    var _setSelectAt = function (pos) {
        ...
        switch (pos) {
            case 'home':
                $select = Global.fileListAll.first();
                break;
            case 'end':
                $select = Global.fileListAll.last();
                break;
            case 'left':
            case 'up':
            case 'right':
    ...
    var fileLight = {
        ...
        getUrl: function ($obj) {
            return $obj.find(".ico").attr("thumb");
        },
        //已有的情况下，选择则标记右键菜单标记
